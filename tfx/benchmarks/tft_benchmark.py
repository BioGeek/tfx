...
def regenerate_intermediates_for_dataset(dataset):
  ...
  with beam.Pipeline(runner=fn_api_runner.FnApiRunner()) as p:
...
class TFTBenchmark(test.Benchmark):
  ...
  def benchmarkAnalyzeAndTransformDataset(self):
    ...
    Runs AnalyzeAndTransformDataset in a Beam pipeline. Records the time taken
    (as reported by Beam) for the Decode, AnalyzeDataset and TransformDataset
    stages, as well as the wall time taken for the whole pipeline.
    """
    ...
    benchmark_utils.report_beam_stages(
        ...
        stages=["Decode", "AnalyzeDataset", "TransformDataset"],
